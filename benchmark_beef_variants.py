...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark Beef dataset across compressed/uncompressed MAT and other formats."
    )
    base = Path(__file__).parent
    parser.add_argument("--mat-file", default=str(base / "Beef.mat"))
    parser.add_argument("--mat-uncompressed-file", default=str(base / "Beef_uncompressed.mat"))
    parser.add_argument("--cube-var", default="Image")
    parser.add_argument("--wave-var", default="Wavelength")
    parser.add_argument("--envi-hdr", default=str(base / "Beef.hdr"))
    parser.add_argument("--envi-dat", default=str(base / "Beef.raw"))
    parser.add_argument("--npy-file", default=str(base / "Beef_Image.npy"))
    parser.add_argument("--zarr-file", default=str(base / "Beef_fixed.ome.zarr"))
    ...
    parser.add_argument("--csv-out", default=str(base / "beef_variants_benchmark.csv"))
    parser.add_argument("--report-out", default=str(base / "beef_variants_report.md"))
...
def build_sources(args: argparse.Namespace) -> list[Source]:
    ...
    npy_reader = CONVERT_HSI.NpyReader(args.npy_file, None, mat_meta.shape[0])
    ...
    sources = [
        ...
        Source(
            ...
            lambda: CONVERT_HSI.NpyReader(args.npy_file, None, mat_meta.shape[0]),
        ),
        Source(
            "omezarr",
            "OME-Zarr",
...
def generate_report(args: argparse.Namespace, sources: list[Source], csv_path: Path, report_path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- `MATLAB compressed` and `MATLAB uncompressed` store the same `float64` data.",
            "- ENVI stores this Beef dataset as `uint16`, so its file size is not directly comparable to the float64 variants.",
            "- CSV: "
